...
static void ag7100_check_link_timer(unsigned long data);
static void ag7100_check_link(ag7100_mac_t *mac);
#ifdef DMA_DEBUG
...
ag7100_open(struct net_device *dev)
{
...
#if defined(CONFIG_ATHRS_PHY)
    /* if using header for register configuration, we have to     */
    /* configure s26 register after frame transmission is enabled */
    if (mac->mac_unit == 1){
        if (mac->mac_phyid == ATHR_S26_PHYID) /* wan phy */
            athrs26_reg_init();
        if (mac->mac_phyid == ATHR_S16_PHYID)
            athrs16_reg_init();
    }
    
    if (mac->mac_phyid == ATHR_S26_PHYID)
        athrs26_phy_setup(mac->mac_unit);
    else if (mac->mac_phyid == ATHR_S16_PHYID)
        athrs16_phy_setup(mac->mac_unit);
    else
        printk(MODULE_NAME ": phyid2 0x%x not supported (0x%x 0x%x).\n", 
               mac->mac_phyid, ATHR_S26_PHYID, ATHR_S16_PHYID );
#elif defined(CONFIG_MACH_AR7100_PB47)
    ...
    */
    ...
    mac->mac_phy_timer.function = ag7100_check_link_timer;
...
#if defined(CONFIG_ATHRS_PHY)
    mac->mac_phyid = ag7100_mii_read(mac->mac_unit, 0, ATHR_PHY_ID2);
    if(mac->mac_phyid == ATHR_S26_PHYID) {
        printk("mac unit %d: using S26 PHY RMII mode\n", mac->mac_unit);
        athrs26_reg_dev(ag7100_macs);
        mii0_if = AG7100_MAC_CFG0_RMII;
        mii1_if = AG7100_MAC_CFG1_RMII;
    }   
    if(mac->mac_phyid == ATHR_S16_PHYID) {
        printk("mac unit %d: using S16 PHY RGMII mode\n", mac->mac_unit);
        mii0_if = AG7100_MAC_CFG0_RGMII;
        mii1_if = AG7100_MAC_CFG1_RGMII;
    }   
#endif
...
ag7100_set_mac_from_link(ag7100_mac_t *mac, ag7100_phy_speed_t speed, int fdx)
{
    ...
    if((mac->mac_phyid == ATHR_S16_PHYID) && !is_setup_done && 
#ifndef CONFIG_PORT0_AS_SWITCH
        mac->mac_unit == 0 && 
#else
        mac->mac_unit == 1 && 
...
#ifdef CONFIG_ATHRS_PHY
    if((mac->mac_phyid == ATHR_S26_PHYID) && change_flag)
        athrs26_phy_on(mac);
#endif
 ...
 */
static void
ag7100_check_link_timer(unsigned long data)
{
    ag7100_mac_t *mac = (ag7100_mac_t *) data;
    /* do the work outside of the timer list */
    schedule_work(&mac->mac_check_link);
}
...
ag7100_check_link(ag7100_mac_t *mac)
{
    ...
    */
    if(mii_if(mac) > 3)
        goto done;
#ifdef CONFIG_ATHRS_PHY
    rc = ag7100_get_link_status(mac, &phy_up, &fdx, &speed);
#else
...
#if defined(CONFIG_ATHRS_PHY) && defined(HEADER_EN)
    if (mac->mac_phyid == ATHR_S26_PHYID) {
...
ag7100_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    ag7100_mac_t *mac = (ag7100_mac_t *)dev->priv;
    if(mac->mac_phyid == ATHR_S16_PHYID)
        return athrs16_ioctl(ifr->ifr_data, cmd);

    if(mac->mac_phyid == ATHR_S26_PHYID)
        return athrs26_ioctl(ifr->ifr_data, cmd);
...
ag7100_init(void)
{
    ...
    */
    ...
    for(i = 0; i < AG7100_NMACS; i++)
    {
        ...
        * watchdog task
        */
        INIT_WORK(&mac->mac_tx_timeout, (void *) ag7100_tx_timeout_task, mac);

        /* link setup outside of timer list */
        INIT_WORK(&mac->mac_check_link,  (void *) ag7100_check_link, mac);
