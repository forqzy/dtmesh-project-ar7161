...
irqreturn_t ar7100_wdt_isr(int, void *, struct pt_regs *);

static struct irqaction ar7100_wdt_irq =
    			{ ar7100_wdt_isr, SA_INTERRUPT, { { 0, } },
			  "ar7100_wdt_isr", NULL, NULL };
...
irqreturn_t ar7100_wdt_isr(int cpl, void *dev_id, struct pt_regs *regs)
{
	unsigned delay;
...
#else
	wdt->action = AR7100_WD_ACT_RESET;
#endif
	wdt->can_close = 0;
	init_waitqueue_head(&wdt->wq);

	ar7100_set_wd_timer(wdt->tmo);
	ar7100_set_wd_timer_action(wdt->action);
...
static struct file_operations ar7100wdt_fops = {
	read:	ar7100wdt_read,
...
static struct miscdevice ar7100wdt_miscdev = {
	WATCHDOG_MINOR,
	"watchdog",
	&ar7100wdt_fops
};
