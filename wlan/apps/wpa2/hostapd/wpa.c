...
#ifdef EAP_WPS
        #if 0 /* Note: noone uses wps_state, might be useful for debugging? */
                                    ...
                                    const u8 *replay_counter)
{
    ...
    for (i = 0; i < RSNA_MAX_EAPOL_RETRIES; i++) {
        if (!sm->key_replay[i].valid)
            break;
        if (memcmp(replay_counter, sm->key_replay[i].counter,
...
/* wpa_wps_fix -- called when we determine late that we really
*       want to do WPS and not e.g. WPA-PSK
*/
void wpa_wps_fix(struct wpa_state_machine *sm)
{
	sm->wpa_key_mgmt = WPA_KEY_MGMT_IEEE8021X;
        sm->wpa = WPA_VERSION_NO_WPA;
        sm->pairwise = WPA_CIPHER_NONE;
        free(sm->wpa_ie);
        sm->wpa_ie = NULL;
        return;
}
...
#if 0   /* removing this feature, since i don't understand it -Ted */
/* wpa_wps_prepare is called from driver_madwifi.c when it sees no valid
 * WPA or RSN information element.... what does this have to do with WPS?
 * this is called INSTEAD of calling wpa_validate_wpa_ie ...
