...
static const char fuser_options[]="mks64";
static long fuser_option_mask = 0;
enum {
	OPT_MOUNT  = (1 << 0),
	OPT_KILL   = (1 << 1),
	OPT_SILENT = (1 << 2),
				...
				unsigned port, inode_list *ilist)
{
	...
	while (fgets(line, MAX_LINE, f)) {
		if (sscanf(line, "%*d: %64[0-9A-Fa-f]:%x %*x:%*x %*x %*x:%*x "
			...
			if (strlen(addr) == 8 && (fuser_option_mask & OPT_IP6))
				continue;
			if (strlen(addr) > 8 && (fuser_option_mask & OPT_IP4))
				continue;
			if (tmp_port == port) {
...
static int search_dev_inode(inode_list *ilist, dev_t dev, ino_t inode)
{
	while (ilist) {
		if (ilist->dev == dev) {
			if (fuser_option_mask & OPT_MOUNT)
				return 1;
...
static pid_list *scan_proc_pids(inode_list *ilist)
{
	...
	plist = NULL;
	while ((de = readdir(d)) != NULL) {
	    if(safe_strtoi(de->d_name, (int *) &pid))
			continue;
...
*/
	...
	fuser_option_mask = bb_getopt_ulflags(argc, argv, fuser_options);
	argv += optind;
	ilist = NULL;
	pp = argv;
	while (*pp) {
	...
	if (!plist)
		return EXIT_FAILURE;
	success = 1;
	if (fuser_option_mask & OPT_KILL) {
		success = kill_pid_list(plist, killsig);
	} else if (!(fuser_option_mask & OPT_SILENT)) {
