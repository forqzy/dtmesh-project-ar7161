...
#define AH_TIME_QUANTUM        100     /* usec */
	...
	for (i = AH_TX_STOP_DMA_TIMEOUT / AH_TIME_QUANTUM; i != 0; i--) {
...
void
ar5212ClearTxDescStatus(struct ath_hal *ah, struct ath_desc *ds)
{
	struct ar5212_desc *ads = AR5212DESC(ds);
	ads->ds_txstatus0 = ads->ds_txstatus1 = 0;
}
...
#ifdef ATH_SWRETRY
void 
ar5212ClearDestMask(struct ath_hal *ah, struct ath_desc *ds)
{
	struct ar5212_desc *ads = AR5212DESC(ds);
 ...
 */
u_int32_t
ar5212CalcTxAirtime(struct ath_hal *ah, struct ath_desc *ds)
{
    return 0;
}
