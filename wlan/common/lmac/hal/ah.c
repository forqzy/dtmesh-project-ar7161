...
extern  struct ath_hal *ar5312Attach(u_int16_t,
    HAL_ADAPTER_HANDLE, HAL_SOFTC,
    HAL_BUS_TAG, HAL_BUS_HANDLE, HAL_BUS_TYPE,
    struct hal_reg_parm *, HAL_STATUS*);
#endif
    ...
    struct hal_reg_parm *hal_conf_parm, HAL_STATUS *error)
{
    ...
    case AR5212_AR2317_REV1:
    case AR5212_AR2317_REV2:
        ah = ar5312Attach(devid, osdev, sc, st, sh, bustype, hal_conf_parm, error);
        break;
#endif
#ifdef AH_SUPPORT_AR5212
    case AR5212_DEVID_IBM:
    ...
    case AR5212_FPGA:
    case AR5212_DEFAULT:
        ah = ar5212Attach(devid, osdev, sc, st, sh, bustype, hal_conf_parm, error);
        break;
#endif
#ifdef AH_SUPPORT_AR5211
    case AR5211_DEVID:
    ...
    case AR5211_FPGA11B:
    case AR5211_DEFAULT:
        ah = ar5211Attach(devid, osdev, sc, st, sh, bustype, hal_conf_parm, error);
        break;
#endif
#ifdef AH_SUPPORT_AR5210
    case AR5210_AP:
    case AR5210_PROD:
    case AR5210_DEFAULT:
        ah = ar5210Attach(devid, osdev, sc, st, sh, bustype, hal_conf_parm, error);
        break;
#endif
#ifdef AH_SUPPORT_AR5416
        case AR5416_DEVID_PCI:
	...
	case AR5416_DEVID_AR9287_PCI:
	case AR5416_DEVID_AR9287_PCIE:
                ah = ar5416Attach(devid, osdev, sc, st, sh, bustype, hal_conf_parm, error);
                break;
#endif
    default:
...
static const u_int8_t CLOCK_RATE[]  = { 40,  80,   22,  44,   88,  40, 40, 44 };
...
{
        const HAL_CHANNEL *c = (const HAL_CHANNEL *) AH_PRIVATE(ah)->ah_curchan;
        u_int8_t clockrateMHz = CLOCK_RATE[ath_hal_chan2htwmode(ah,c)];
    ...
    u_int32_t capability, u_int32_t *result)
{
    ...
    case HAL_CAP_MISCFLAGS:
        *result=AH_PRIVATE(ah)->ah_miscFlags;
        return HAL_OK;
#endif
    ...
    case HAL_CAP_MBSSID_AGGR_SUPPORT:   
        *result = pCap->halNumAggrVapSupport;
        return  HAL_OK;
    ...
    case HAL_CAP_WEP_TKIP_AGGR_RX_DELIM:
        *result = pCap->halWepTkipAggrNumRxDelim;
        return HAL_OK;
    case HAL_CAP_HIGH_PWR_ANT:
        *result = pCap->halHighPwrAnt;
        return HAL_OK;
    case HAL_CAP_SINGLE_AGGR_SAFE:   
        return pCap->halSingleFrameAggrSupport ? HAL_OK : HAL_ENOTSUPP;
...
#ifdef ATH_HAL_CAP_MISCFLAGS
    case HAL_CAP_MISC_FLAGS:
        AH_PRIVATE(ah)->ah_miscFlags=setting;
        return AH_TRUE;
#endif
...
ath_hal_process_noisefloor(struct ath_hal *ah)
{
    ...
    for (i = 0; i < 64; i++) {
        ...
        HDPRINTF(ah, HAL_DBG_NF_CAL, "%u/0x%x raw nf %d adjust %d\n",
...
HAL_BOOL ath_hal_enabledANI(struct ath_hal *ah)
{
                         ...
                         void *valBuff)
{
    ...
    */
    ...
    switch(p)
    {
        ...
        break;
#endif
        case HAL_CONFIG_RXMIT_LASTPKT:
            ah->ah_SetIntrMitigationTimer(ah, HAL_INT_RX_LASTPKT,
                                          *(int *)valBuff);
        break;
        case HAL_CONFIG_RXMIT_FIRSTPKT:
            ah->ah_SetIntrMitigationTimer(ah, HAL_INT_RX_FIRSTPKT,
                                          *(int *)valBuff);
        break;
        case HAL_CONFIG_TXMIT_LASTPKT:
            ah->ah_SetIntrMitigationTimer(ah, HAL_INT_TX_LASTPKT,
                                          *(int *)valBuff);
        break;
        case HAL_CONFIG_TXMIT_FIRSTPKT:
            ah->ah_SetIntrMitigationTimer(ah, HAL_INT_TX_FIRSTPKT,
                                          *(int *)valBuff);
        break;
...
#endif
        case HAL_CONFIG_RXMIT_LASTPKT:
            *(int *)valBuff = ah->ah_GetIntrMitigationTimer(ah,
                                                           HAL_INT_RX_LASTPKT);
            retval = sizeof(int);
        break;
        case HAL_CONFIG_RXMIT_FIRSTPKT:
...
#ifdef ATH_CCX
void
ath_hal_get_sernum(struct ath_hal *ah, u_int8_t *pSn)
{
    struct ath_hal_private *ahp = AH_PRIVATE(ah);
    u_int8_t    i;

    for(i=0; i<sizeof(ahp->serNo); i++){
        pSn[i] = ahp->serNo[i];
    }
}    
