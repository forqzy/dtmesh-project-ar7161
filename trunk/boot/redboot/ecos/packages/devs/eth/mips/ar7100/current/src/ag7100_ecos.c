...
typedef struct ag7100_priv_data_s {
    ...
    ag7100_desc_t  tx_desc[AG7100_TX_DESC_CNT] __attribute__ ((aligned (HAL_DCACHE_LINE_SIZE)));
    ag7100_desc_t  rx_desc[AG7100_RX_DESC_CNT] __attribute__ ((aligned (HAL_DCACHE_LINE_SIZE)));
} ag7100_priv_data_t;
...
static int
ag7100_alloc_fifo(int ndesc, ag7100_desc_t **fifo, ag7100_desc_t *desc)
{
    int i;
    ag7100_desc_t *p = CYGARC_UNCACHED_ADDRESS(desc);
...
ag7100_setup_fifos(ag7100_mac_t *mac)
{
    ...
    if (ag7100_alloc_fifo(AG7100_TX_DESC_CNT, mac->fifo_tx, ag7100_priv->tx_desc))
        return 1;
    ...
    if (ag7100_alloc_fifo(AG7100_RX_DESC_CNT, mac->fifo_rx, ag7100_priv->rx_desc))
        return 1;
...
#endif
#ifdef CONFIG_ATHRS16_PHY 
    if(!is_setup_done && mac->mac_unit == 1 && (mac->mac_speed != speed || mac->mac_duplex != fdx)) 
    {   
       phy_mode_setup();
       is_setup_done = 1;
    }
