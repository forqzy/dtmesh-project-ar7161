...
** set_mac
**
** Set and read mac address
**
*/

static cmd_fun do_set_mac;
RedBoot_cmd("set_mac","Set/Read MAC address for ethernet ports","[-p <portnum>] [xx:xx:xx:xx:xx:xx]\n",do_set_mac);
    
static void
do_set_mac (int argc, char *argv[]){
    bool portnum_set;
    int  portnum, i;
    char *addr = 0;
    ...
    if (!portnum_set) {
        diag_printf("WAN eth0 mac: %02x:%02x:%02x:%02x:%02x:%02x\n",
                    ath_cfg.MAC1[0],ath_cfg.MAC1[1],ath_cfg.MAC1[2],ath_cfg.MAC1[3],ath_cfg.MAC1[4],ath_cfg.MAC1[5]
                   );
        diag_printf("LAN eth1 mac: %02x:%02x:%02x:%02x:%02x:%02x\n",
                    ath_cfg.MAC2[0],ath_cfg.MAC2[1],ath_cfg.MAC2[2],ath_cfg.MAC2[3],ath_cfg.MAC2[4],ath_cfg.MAC2[5]
                   );
        return;
    }
    ...
    for (i = 0; i < 6; i++) {
        if (!_is_hex(addr[0]) || !_is_hex(addr[1]))
            break;
	if(portnum == 0)
            ath_cfg.MAC1[i] = (_from_hex(addr[0]) * 16) + _from_hex(addr[1]);
        else if(portnum == 1)
            ath_cfg.MAC2[i] = (_from_hex(addr[0]) * 16) + _from_hex(addr[1]);
        addr += 2;
        if (*addr == ':')
            addr++;
    }
...
#if (CYGNUM_FLASH_BLOCK_NUM == 128)
#define ATH_CFG_ADDR  0xbf7f0000  /* 8MB version */
#else 
#define ATH_CFG_ADDR  0xbfff0000  /* 16MB version */
#endif
