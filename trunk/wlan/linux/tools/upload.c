...
void ShowOkPage(void) {
  ...
  while (fgets(Line,BUFSIZ,Fin) != NULL) {
    ...
    while (*p1 != '\0') {
      ...
      if (strnicmp(p1,"<insert dirname>",16) == 0) {
        *p1 = '\0';
        sprintf(s1,"%s%s%s",Line,Root,p1 + 16);
        strcpy(Line,s1);
        }
      p1++;
      }
...
void AcceptSegment(char **Input,            /* Pointer into the incoming stream. */
  ...
  *InputLength = *InputLength - strlen(Boundary);
  ...
  sprintf(s1,"File uploaded successfully: %s (%lu bytes)",
...
int main(int argc, char *argv[], char *environment[]) {
  ...
  *p1 = '\0';
  ...
  if (Debug > 0) fprintf(stdout,"Input successfully loaded into memory.<br>\n");
