 ...
 *
 * $Id: //depot/sw/releases/7.3_AP/wlan/linux/tools/hal_diag/dumpregs.c#3 $
 */
#include "diag.h"
...
struct	ath_diag atd;
int	s;
#define RANGE_FORMAT_DFLT 0
#define RANGE_FORMAT_ART 1
...
main(int argc, char *argv[])
{
	...
	what = 0;
	while ((c = getopt(argc, argv, "I:abdgkilpqxF")) != -1)
		switch (c) {
                case 'F':
                        rangeformat=RANGE_FORMAT_ART;
                        break;
		case 'a':
			what |= DUMP_ALL;
			break;
...
ath_hal_dumprange(FILE *fd, u_int a, u_int b)
{
	u_int r;
        ...
        if(rangeformat == RANGE_FORMAT_DFLT) {
	...
	case 4:
        ...
        if(rangeformat == RANGE_FORMAT_ART) {
                for (r = a; r <= b; r += 4)
                    fprintf(fd, "%04x %08x\n" , r, OS_REG_READ(ah, r));
        }
}
