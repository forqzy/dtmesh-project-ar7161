...
sysFlashConfigInit_x16(FlashWPTR ptr)
{
    ...
    cfi.size = 0;
    cfi.base = (UINT8 *)(ioremap(ptr, 0x10000));
...
printk("cfi.blockSize is 0x%x\n", cfi.blockSize);
    ...
    basePtr = (UINT8 *)(ioremap(base, cfi.ebSize[off]));
    i = NFLC - 1;
...
printk("i=0:fPtr=0x%x:fS=0x%x:ebSize=0x%x\n", cfi.flcPtr[0], cfi.flcSize[0], cfi.ebSize[off]);
printk("i=1:fPtr=0x%x:fS=0x%x:ebSize=0x%x\n", cfi.flcPtr[1], cfi.flcSize[1], cfi.ebSize[off]);
printk("i=2:fPtr=0x%x:fS=0x%x:ebSize=0x%x\n", cfi.flcPtr[2], cfi.flcSize[2], cfi.ebSize[off]);
...
unsigned int dk_flash_write (int flc, unsigned int offset, size_t len,const UINT8 * buf) {
