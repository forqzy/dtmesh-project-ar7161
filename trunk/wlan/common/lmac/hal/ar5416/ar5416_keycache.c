                       ...
                       int xorKey)
{
	...
	if (AH_PRIVATE(ah)->ah_curchan == AH_NULL) {
		...
		/* remember that a wep key was configured
		 * RIFS shall be disabled later when the channel is allocated 
		 */
        	ahp->ah_rifs_sec_cnt++;
	        HDPRINTF(ah, HAL_DBG_KEYCACHE, "%s: Count = %d, not disabling RIFS- channel NULL \n", __func__, ahp->ah_rifs_sec_cnt);  
