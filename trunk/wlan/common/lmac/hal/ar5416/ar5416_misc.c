        ...
        u_int32_t capability, u_int32_t *result)
{
        ...
        case HAL_CAP_BB_RIFS_HANG:
                 ...
                 * XXX: Add Merlin
                 */
                return (AR_SREV_SOWL(ah) || AR_SREV_HOWL(ah)) ?
                        HAL_OK : HAL_ENOTSUPP;
       ...
       case HAL_CAP_RIFS_RX_ENABLED:
        ...
        case HAL_CAP_DYNAMIC_SMPS:
                return AR_SREV_MERLIN_20(ah) ? HAL_OK : HAL_ENOTSUPP;
        case HAL_CAP_DS:
...
ar5416PrintReg(struct ath_hal *ah, u_int32_t args)
{
    ...
    if (args & HAL_DIAG_PRINT_REG_ALL) {
        ...
        for(i=0x8240; i<=0x8258; i += sizeof(u_int32_t))
        {
            HDPRINTF(ah, HAL_DBG_PRINT_REG, "0x%04x 0x%08x\n", i, OS_REG_READ(ah, i));
        }
...
#define NUM_QUEUES          10
    ...
    for (i = 0; i < NUM_NF_READINGS; i++) {
        ...
        for (j = 0, k = h[i].currIndex; j < HAL_NF_CAL_HIST_MAX; j++, k++) {
            ath_hal_printf(ah, "Element %d: %d\n",
                           j, h[i].nfCalBuffer[k%HAL_NF_CAL_HIST_MAX]);
        }
...
#define N(a) (sizeof(a)/sizeof(a[0]))
    struct ath_hal_5416 *ahp = AH5416(ah);
    u_int32_t hang_sig = 0, hang_num = 0;
    HAL_BOOL hang_found = AH_FALSE;
    int i=0;
    ...
    hal_hw_hang_check_t hang_list [] =
    {
     /* Offset        Reg Value   Reg Mask    Hang Offset */
       ...
       {AR_OBS_BUS_1, 0x00702400, 0x7FF3FFEF, BB_HANG_SIG4},
       {AR_OBS_BUS_1, 0x18002313, 0x7EF3FFFF, BB_HANG_SIG5},
       {AR_OBS_BUS_1, 0x00902400, 0x5DF3FFEE, BB_HANG_SIG6},
    };
    ...
    ahp->ah_hang[hang_list[hang_num].hang_offset]++;
    HDPRINTF(ah, HAL_DBG_DFS, "%s: hang_sig = 0x%x\n", __func__, hang_sig);
    HDPRINTF(ah, HAL_DBG_DFS, "%s sig1count=%d sig2count=%d sig3count=%d "
                              "sig4count=%d sig5count=%d sig6count=%d\n",
                   __func__,
                   ahp->ah_hang[BB_HANG_SIG1], ahp->ah_hang[BB_HANG_SIG2],
                   ahp->ah_hang[BB_HANG_SIG3], ahp->ah_hang[BB_HANG_SIG4],
                   ahp->ah_hang[BB_HANG_SIG5], ahp->ah_hang[BB_HANG_SIG6]);
...
u_int ar5416GetMacVersion(struct ath_hal *ah)
{
	return ah->ah_macVersion;
}
#ifdef ATH_BT_COEX
void
ar5416GetBTCoexInfo(struct ath_hal *ah, HAL_BT_COEX_INFO *btinfo)
{
    struct ath_hal_5416 *ahp = AH5416(ah);

    btinfo->bt_module = ahp->ah_btModule;
    btinfo->bt_gpioBTActive = ahp->ah_btActiveGpioSelect;
    btinfo->bt_gpioBTPriority = ahp->ah_btPriorityGpioSelect;
    btinfo->bt_gpioWLANActive = ahp->ah_wlanActiveGpioSelect;
    btinfo->bt_activePolarity = ahp->ah_btActivePolarity;
}
...
ar5416BTCoexConfig(struct ath_hal *ah, HAL_BT_COEX_CONFIG *btconf)
{
    ...
    ahp->ah_btCoexMode = (ahp->ah_btCoexMode & AR_BT_QCU_THRESH) |
        ...
        SM(btconf->bt_rxclearPolarity, AR_BT_RX_CLEAR_POLARITY) |
...
void
ar5416BTCoexSetWeights(struct ath_hal *ah, u_int32_t bt_weight, u_int32_t wlan_weight)
{
    struct ath_hal_5416 *ahp = AH5416(ah);
    ahp->ah_btCoexWeights = SM(bt_weight, AR_BT_BT_WGHT) |
        SM(wlan_weight, AR_BT_WL_WGHT);
}
...
ar5416BTCoexDisable(struct ath_hal *ah)
{
    ...
    ath_hal_gpioCfgOutput(ah, ahp->ah_wlanActiveGpioSelect, AR_GPIO_OUTPUT_MUX_AS_OUTPUT);
...
ar5416BTCoexEnable(struct ath_hal *ah)
{
    ...
    OS_REG_WRITE(ah, AR_BT_COEX_WEIGHT, ahp->ah_btCoexWeights);
    OS_REG_WRITE(ah, AR_BT_COEX_MODE2, ahp->ah_btCoexMode2);

    /* Configure the desired GPIO port for TX_FRAME output */
    /* Only needed when Reg0x4060 output mux is set to 9.
     * OS_REG_WRITE(ah, AR_PHY(2), OS_REG_READ(ah, AR_PHY(2)) & (~0x60000000));
     * OS_REG_WRITE(ah, AR_OBS, 0x0b);
     */
...
ar5416AntDivCombGetConfig(struct ath_hal *ah, HAL_ANT_COMB_CONFIG* divCombConf) {
    ...
    divCombConf->fast_div_bias = ((regVal & AR_PHY_KITE_ANT_FAST_DIV_BIAS) >> 
                                   AR_PHY_KITE_ANT_FAST_DIV_BIAS_S);
}
...
ar5416AntDivCombSetConfig(struct ath_hal *ah, HAL_ANT_COMB_CONFIG* divCombConf) {
    ...
    regVal |= ((divCombConf->alt_lna_conf << AR_PHY_KITE_ANT_DIV_ALT_LNACONF_S) &
               AR_PHY_KITE_ANT_DIV_ALT_LNACONF);
    regVal |= ((divCombConf->fast_div_bias << AR_PHY_KITE_ANT_FAST_DIV_BIAS_S) &
               AR_PHY_KITE_ANT_FAST_DIV_BIAS);
