	...
	u_int32_t next_beacon, u_int32_t beacon_period)
{
	...
	case HAL_M_IBSS:
		...
		OS_REG_WRITE(ah, AR_NEXT_NDP_TIMER,
			TU_TO_USEC(next_beacon + (ahp->ah_atimWindow ? ahp->ah_atimWindow : 1)));
		flags |= AR_NDP_TIMER_EN;
	case HAL_M_HOSTAP:
		OS_REG_WRITE(ah, AR_NEXT_TBTT_TIMER, TU_TO_USEC(next_beacon));
		OS_REG_WRITE(ah, AR_NEXT_DMA_BEACON_ALERT,
			TU_TO_USEC(next_beacon - ap->ah_config.ath_hal_dma_beacon_response_time));
		OS_REG_WRITE(ah, AR_NEXT_SWBA,
			TU_TO_USEC(next_beacon - ap->ah_config.ath_hal_sw_beacon_response_time));
	...
	OS_REG_WRITE(ah, AR_NDP_PERIOD, TU_TO_USEC(beacon_period));
