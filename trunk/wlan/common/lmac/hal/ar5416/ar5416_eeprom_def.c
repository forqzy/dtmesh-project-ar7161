...
static inline HAL_BOOL ar5416EepromDefSetPowerPerRateTable(struct ath_hal *ah,
...
ar5416EepromDefSetBoardValues(struct ath_hal *ah, HAL_CHANNEL_INTERNAL *chan)
{
    ...
    if (AR_SREV_MERLIN_10_OR_LATER(ah)) {
        if (IS_CHAN_2GHZ(chan)) {
            analagShiftRegRMW(ah, AR_AN_RF2G1_CH0, AR_AN_RF2G1_CH0_OB, AR_AN_RF2G1_CH0_OB_S, pModal->ob);
            analagShiftRegRMW(ah, AR_AN_RF2G1_CH0, AR_AN_RF2G1_CH0_DB, AR_AN_RF2G1_CH0_DB_S, pModal->db);
            analagShiftRegRMW(ah, AR_AN_RF2G1_CH1, AR_AN_RF2G1_CH1_OB, AR_AN_RF2G1_CH1_OB_S, pModal->ob_ch1);
            analagShiftRegRMW(ah, AR_AN_RF2G1_CH1, AR_AN_RF2G1_CH1_DB, AR_AN_RF2G1_CH1_DB_S, pModal->db_ch1);
        } else {
            analagShiftRegRMW(ah, AR_AN_RF5G1_CH0, AR_AN_RF5G1_CH0_OB5, AR_AN_RF5G1_CH0_OB5_S, pModal->ob);
            analagShiftRegRMW(ah, AR_AN_RF5G1_CH0, AR_AN_RF5G1_CH0_DB5, AR_AN_RF5G1_CH0_DB5_S, pModal->db);
            analagShiftRegRMW(ah, AR_AN_RF5G1_CH1, AR_AN_RF5G1_CH1_OB5, AR_AN_RF5G1_CH1_OB5_S, pModal->ob_ch1);
            analagShiftRegRMW(ah, AR_AN_RF5G1_CH1, AR_AN_RF5G1_CH1_DB5, AR_AN_RF5G1_CH1_DB5_S, pModal->db_ch1);
        }
        
        analagShiftRegRMW(ah, AR_AN_TOP2, AR_AN_TOP2_XPABIAS_LVL, AR_AN_TOP2_XPABIAS_LVL_S, pModal->xpaBiasLvl);
        analagShiftRegRMW(ah, AR_AN_TOP2, AR_AN_TOP2_LOCALBIAS, AR_AN_TOP2_LOCALBIAS_S, pModal->local_bias);
    ...
    if (AR_SREV_MERLIN_10_OR_LATER(ah)) {
        ...
        OS_REG_RMW_FIELD(ah, AR_PHY_EXT_CCA, AR_PHY_EXT_CCA_THRESH62, pModal->thresh62);
    }
    ...
    if (AR_SREV_MERLIN_20_OR_LATER(ah) && 
        ((eep->baseEepHeader.version & AR5416_EEP_VER_MINOR_MASK) >= AR5416_EEP_MINOR_VER_19))
    {
        OS_REG_RMW_FIELD(ah, AR_PHY_CCK_TX_CTRL,  AR_PHY_CCK_TX_CTRL_TX_DAC_SCALE_CCK, pModal->miscBits);
    }

    if (AR_SREV_MERLIN_20(ah) && 
        ((eep->baseEepHeader.version & AR5416_EEP_VER_MINOR_MASK) >= AR5416_EEP_MINOR_VER_20))
    {
        if (IS_CHAN_2GHZ(chan)) {
            analagShiftRegRMW(ah, AR_AN_TOP1, AR_AN_TOP1_DACIPMODE, AR_AN_TOP1_DACIPMODE_S, eep->baseEepHeader.dacLpMode);
        } else {
            if (eep->baseEepHeader.dacHiPwrMode_5G) {
                analagShiftRegRMW(ah, AR_AN_TOP1, AR_AN_TOP1_DACIPMODE, AR_AN_TOP1_DACIPMODE_S, 0);
            } else {
                analagShiftRegRMW(ah, AR_AN_TOP1, AR_AN_TOP1_DACIPMODE, AR_AN_TOP1_DACIPMODE_S, eep->baseEepHeader.dacLpMode);
            }
        }
...
ar5416EepromDefSetPowerCalTable(struct ath_hal *ah, ar5416_eeprom_def_t *pEepData, HAL_CHANNEL_INTERNAL *chan, int16_t *pTxPowerIndexOffset)
{
    ...
    pwrTableOffset = (pwrTableOffset == EEP_INVALID_TX_OFFSET) ?
                      AR5416_PWR_TABLE_OFFSET : pwrTableOffset;
    ...
    for (i = 0; i < AR5416_MAX_CHAINS; i++)
    {
        ...
        if (pEepData->baseEepHeader.txMask & (1 << i))
        {
            ...
            if (AR_SREV_MERLIN_20_OR_LATER(ah)) {
                ...
                if (AR5416_PWR_TABLE_OFFSET != pwrTableOffset) {
                    /* get the difference in dB */
                    diff = (u_int16_t)(pwrTableOffset - AR5416_PWR_TABLE_OFFSET);
    ...
    */
    ...
    if (AR_SREV_MERLIN_10_OR_LATER(ah)) {
        for (i = 0; i < Ar5416RateSize; i++) {
            ...
            if (pwrTableOffset == EEP_INVALID_TX_OFFSET)
...
#ifdef EEPROM_DUMP
    ar5416EepDefPrintPowerPerRate(ah, ratesArray);
#endif
                ...
                for (k = 0; k < numXpdGain; k++) {
                    ...
                    for (vpdCount = 0; vpdCount < AR5416_EEPDEF_PD_GAIN_ICEPTS; vpdCount++) {
                        for (channelCount = channelRowCnt; (channelCount < rowEndsAtChan)  && (pChannel[channelCount] != 0xff); channelCount++) {
                            ...
                            if(AR_SREV_MERLIN_10_OR_LATER(ah)) {
                                tempPower += AR5416_PWR_TABLE_OFFSET;
                            }
