...
#define AR71XX_DDR_CTRL_BASE    0x1800000
#define AR71XX_DDR_CTRL_SIZE    0x10000

static void 
__init ar71xx_detect_mem_size(void)
{
    u32 size;
    ioremap_nocache(AR71XX_DDR_CTRL_BASE,AR71XX_DDR_CTRL_SIZE);
...
#ifdef CONFIG_INTELLUS
int ar7100_board_type;
EXPORT_SYMBOL(ar7100_board_type);
int __init
set_board_type(char *str)
{
    if(!strcmp(str, "ja73pf"))
        ar7100_board_type = BT_JA73PF;
    else if(!strcmp(str, "ja76pf"))
        ar7100_board_type = BT_JA76PF;
    else if(!strcmp(str, "ja76hx"))
        ar7100_board_type = BT_JA76HX;
    else if(!strcmp(str, "ja76pf2"))
        ar7100_board_type = BT_JA76PF2;
    else
        ar7100_board_type = BT_UNKNOWN;

    printk("Board type is \"%s\" (%d)\n", str, ar7100_board_type);
    return 1;
}
...
#ifdef CONFIG_INTELLUS
    if(ar7100_board_type == BT_JA73PF) {
        MY_WRITE(0xb8040000, 0x5bf);
        MY_WRITE(0xb8040008, 0x23);
    }
    else if(ar7100_board_type == BT_JA76PF || ar7100_board_type == BT_JA76HX || ar7100_board_type == BT_JA76PF2) {
        MY_WRITE(0xb8040000, 0x5bf);
        MY_WRITE(0xb8040008, 0x1b);
    }
#else
