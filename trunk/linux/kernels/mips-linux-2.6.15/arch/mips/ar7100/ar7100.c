...
#ifdef CONFIG_I2C
#include <linux/i2c.h>
static int ar7100_xfer(struct i2c_adapter *i2c_adap,
        struct i2c_msg msgs[], int num)
{
  ...
  for (ii=0; ii<num; ii++) {
    ...
    if (pmsg->flags & I2C_M_RD ) {
      ...
      ret = ar7100_i2c_raw_read_bytes_from_addr((int) (pmsg->addr << 1), pmsg->buf, (int) pmsg->len);
      if (ret) {
        return -EREMOTEIO;
      }
    } else {
      /* write bytes from buffer */
      ret = ar7100_i2c_raw_write_bytes_to_addr((int) (pmsg->addr << 1), pmsg->buf, (int) pmsg->len);
      if (ret) {
        return -EREMOTEIO;
      }
    }
...
static u32 ar7100_func(struct i2c_adapter *adap)
{
  return I2C_FUNC_I2C | I2C_FUNC_SMBUS_EMUL | I2C_FUNC_SMBUS_BYTE_DATA;
}

static struct i2c_algorithm i2c_ar7100_algo = {
  .master_xfer  = ar7100_xfer,
  .functionality  = ar7100_func,
};

static struct i2c_adapter adap_ar7100 = {
  .name = "ar7100 adapter",
  .algo = &i2c_ar7100_algo,
  .class = I2C_CLASS_HWMON,
  .timeout = 100,
  .retries = 3,
};

static int __init ar7100_i2c_init(void)
{
  i2c_add_adapter(&adap_ar7100);

  return 0;
}

static void  __exit ar7100_i2c_exit(void)
{
  i2c_del_adapter(&adap_ar7100);
}

module_init(ar7100_i2c_init);
module_exit(ar7100_i2c_exit);
#endif
